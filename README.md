...
- On a **painted scaffold** block (hover anywhere on it): `Tab` picks up the entire painted block (enters edit mode if needed); `Space` inverts the whole block; left click places it; `Q` undoes.
